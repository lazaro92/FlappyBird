...
namespace Sonar
{
	Bird::Bird(GameDataRef data): _data(data)
	{
		...
		// get the center of the bird sprite to rotate from this position.
	...
	void Bird::Update(float dt)
	{
		...
		{
			_birdSprite.move(0, -FLYNG_SPEED * dt);
			_rotation -= ROTATION_SPEED * dt;
			if (_rotation < -25.0f)
			{
				_rotation = -25.0f;
		...
		if (_movementClock.getElapsedTime().asSeconds() > FLYNG_DURATION)
		{
